...
namespace PaintIn3D
{
	...
	public abstract class P3dPaintableTextureMonitorMask : P3dPaintableTextureMonitor
	{
		...
		protected void UpdateTotal(RenderTexture temporary, int width, int height, int depth, RenderTextureFormat format, int scale)
		{
			if (mesh != null)
			{
				if (baked == false || bakedMesh != mesh || bakedSize.x != width || bakedSize.y != height)
				{
					Bake(temporary, width, height, depth, format, scale);
				}
			}
			else
			{
